...
This transforms each I/O pair to (for example) $\langle\texttt{(f2(f1(), 'Metalica'))}, \texttt{[...]}\rangle$.
...
A total
of six plugins (\ttt{input}, \ttt{graph}, \ttt{signature}, \ttt{pure}, \ttt{trace\_sql}, and \ttt{llm_translate})
...
% Plugin: signature ---
can(signature(p)).
% ---  Plugin: llm_translate ---
can(llm_translate(p, haskell)).
  do(language(p, haskell)) :- do(llm_translate(p, haskell)).
% ---- Axioms ----
...
\end{minted}
  ...
  (2) three plugins (\ttt{len}, \ttt{signature}, and \ttt{llm\_translate}),
...
A \ttt{llm_translate} plugin given that the output program must be written in Haskell, may prepend:
...
Checkers are modular and contributed by plugins.
Their combined results determine whether a candidate is considered valid under the current plan.
...
Its important that certain properties that are not easily verifiable, such as \ttt{pure}, 
do not correspond to a specific verifier, but rather influence what kind of propoerties can be extracted 
otherwise to make sure they are not violated during regeneration.
For this example, a system call trace of the program will not be provided to the LLM instance, 
as that could encode side-effects present in the original program.
...
\begin{itemize}
  ...
  \item \emph{Feedback loop}: \sys will provide the previous regeneration attempt 
    ...
    A key assumption is that the output will not to taint followup regeneration attempts,
    which should hold based on the synthesis plan, which takes query and plugin constraints into account.
  \item \emph{Fallback handling}: If a function fails validation three times, \sys leaves the original version intact or mark it for manual inspection.
\end{itemize}
...
For the flatmap-stream attack, \sys uses domain-specific logic (encoded in its knowledge-base by the \ttt{js-stream} plugin) to generate a function that produces a node \ttt{Stream} object.
...
Here, I outline possible future directions for \sys and its successors:
...
These instances provide interesting insights into the capabilities of \sys and its shortcomings.
