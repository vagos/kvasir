\documentclass[noacm,sigplan,review]{acmart}
...
\subsection{Property Language and Extraction}
\label{sec:dsl}
...
\subsection{Property Verification and Feedback Loop}
...
SSCA                               & Sneaky supply-chain attacks        & 3  &      & Attack elimination     & \cite{ev:eurosec:2022, ohm2020backstabber,copeland2019frightening} \\
IOCCC                              & Obfuscated C code       & 2  &      & De-obfuscation         & \cite{ioccc} \\
Microbenchmarks                    &                    & 3  &      &                        & \\
\hspace{.5em} \ttt{flatmap-stream} & Flat-map utility   &    &      & Purity and translation & \cite{es1}  \\
\hspace{.5em} \ttt{Q\_rsqrt}       & Inverse square root function       &    &      & Idiomatic rewrite      & \cite{fast_inv_sqrt}  \\
\hspace{.5em} \textsf{rigidDB}     & Fetch artist songs from a music database     &    &      & Modularization         & \cite{codewithsadeemusicplayer} \\
\midrule
Total                              &                    & 37 &      &                        & \\
\bottomrule
\end{tabular*}
