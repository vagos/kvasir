...
Additionally, the system minimizes the number of extracted properties:
\[
\text{minimize} \left|\left\{ X \mid \mathsf{can}(X) \right\}\right|
\]
...
\heading{Abstract and concrete space}
...
\midrule
Rosetta Code                       & Programming tasks   & 16 & Language translation                       & \cite{rosettacode} \\
Short utilities                    & npm and Pypi utilities & 13 & Pure and idiomatic regeneration    & \cite{regbench2025} \\
SSCA                               & Sneaky SSAs         & 3  & Malicious code elimination                 & \cite{ev:eurosec:2022, ohm2020backstabber,copeland2019frightening} \\
IOCCC                              & Obfuscated C code   & 2  & De-obfuscation                             & \cite{ioccc} \\
Microbenchmarks                    &                     & 3  &                                            & \\
\hspace{.5em} \ttt{flatmap-stream} & Secure regeneration &    & Pure regeneration and translation & \cite{es1}  \\
\hspace{.5em} \ttt{Q\_rsqrt}       & C refactoring       &    & Idiomatic rewrite                          & \cite{fast_inv_sqrt}  \\
\hspace{.5em} \textsf{rigidDB}     & Modularization      &    & Modularization                             & \cite{codewithsadeemusicplayer} \\
\bottomrule
\end{tabular}
