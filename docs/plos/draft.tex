...
\begin{wrapfigure}[2]{r}{.55\columnwidth}
\begin{minted}{prolog}
graph(p_,I,O):-graph(p,I,O).
#min N: len(p_, N).
\end{minted}
...
We evaluate \sys across several dimensions: its
...
The benchmark suite comprises 37 tasks spanning simple translation, obfuscation removal, and security hardening (Table~\ref{tab:benchmarks}). Inputs were drawn from Rosetta Code, popular npm and PyPi packages, known supply-chain attacks, and selected obfuscated programs from the IOCCC.
