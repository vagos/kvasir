...
The following query for \sys uses lines-of-code a coarse metric of idiomaticity.
\sys will regenerate the program, minimizing this metric.
\begin{wrapfigure}[3]{r}{.5\columnwidth}
  \vspace{-10pt}
  \begin{minted}{prolog}
equal(io(P), io(P_)).
#min len(P_).
  \end{minted}
\end{wrapfigure}
\sys extracts the $\{\ttt{sig(F)}, \ttt{io(F)}\}$ properties from the original program, where \texttt{sig(F)} is the
...
These results support the claim that \sys’s planning logic — driven by explicit property extraction and knowledge-base reasoning — yields safer and more controllable transformations than prompt-only LLM-based systems.
