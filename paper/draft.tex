...
\begin{itemize}
  \item A signature plugin may contribute: \emph{\say{The output should implement a function \texttt{sqrt(n: float): float}.}}
  \item A length plugin combined with a minimization goal may prepend: \emph{\say{The output should be as short as possible while preserving the original functionality.}}
  \item A language plugin may prepend: \emph{\say{The output function should be written in Haskell.}}
\end{itemize}
...
Supporting a new language requires implementing language-specific analyzers and constraints. This modularity is deliberate, but it does limit out-of-the-box generality. The current \sys prototype supports Python, JavaScript, Haskell, and C.
